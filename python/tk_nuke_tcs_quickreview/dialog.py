...
class Dialog(QtGui.QWidget):
    ...
    def _render(self, mov_path, start_frame, end_frame):
        ...
        mov_out = self._group_node.node("mov_writer_tcs")
    ...
    def _upload_to_shotgun(self, shotgun, data):
        ...
        logger.debug("Uploading movie to ShotGrid with ACES setup...")
        try:
            shotgun.upload(
            ...
            logger.exception("shotgun upload could not be performed")

        # finally:
        #     pathToInputFile = mov_path["file"].value()
        #     mov_file_current_time = self._format_timestamp(pathToInputFile).datetime_now()
        #     today = date.today()
        #     if is_one_month(date(mov_file_current_time), today):
    ...
    def _run_submission(self):
        ...
        # try:
        #     version_number = publisher.util.get_file_path_components("version_number")
        # except Exception as e:
        #     logger.exception("Version Number not recognised in the file name ")
        ...
        # get the set group node exr file path and mov file path for the quicktime movie swap in RV
        # could be used with the MediaSwap plugin for RV as well

        folderName = ""
        folderPath = ""
        exrPath = ""
        groupNode = nuke.toNode("ShotGridQuickReviewTcs")

        for key, value in groupNode.metadata().iteritems():
            if key == "input/filename":
                exrPath = value
                folderPath = os.path.dirname(value)
                folderName = os.path.basename(value)

        tmp = folderName.split(".")
        movFileName = tmp[0]+"."+tmp[1]+".mov"

        mov_name = str(movFileName)
        mov_path = os.path.join(folderPath,mov_name)

        logger.info("mov path set to  %s" % mov_path)
