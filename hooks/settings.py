    ...
    def setup_quicktime_node(self, write_node):
        ...
        if sgtk.util.is_linux():
            if nuke.NUKE_VERSION_MAJOR >= 9:
                ...
                # settings for color management in project settings while using ocio config
                ...
                if color_management == "OCIO":
                    ...
                    # nuke.addOnCreate(nuke.Root().knob("OCIO_config").setValue('reloadConfig'))
                    # nuke.addKnobChanged(nuke.Root().knob("OCIO_config").setValue('reloadConfig'))
            else:
